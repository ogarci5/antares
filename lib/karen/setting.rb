module Karen
  module Setting
    def self.load!
      ...
      Karen::Redis.set('setting:messages_reminder', messages[:reminder].times.to_a)
      messages[:reminder].each_with_index do |message, index|
        Karen::Redis.set("setting:messages_reminder:#{index+1}", message.merge(id: index+1))
      end
      Karen::Redis.set('setting:messages_reminder', messages[:slack]

      Karen::Redis.set('setting:messages', messages)
      Karen::Redis.set('setting:reminders', reminders)
    end
